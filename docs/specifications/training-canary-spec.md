...
| 5.9.0 | 2026-04-04 | **Yoga CUDA NF4 completes in 7079s (2 hours) — not a hang, pathologically slow JIT.** Second yoga run confirms: process completes but takes 2 hours (3.8 tok/s with JIT overhead). CUDA driver cache exists (415 MB at ~/.nv/ComputeCache/) but doesn't help — second run equally slow. Five-whys: trueno may generate unique PTX each invocation (different compile flags or inline constants), defeating driver cache. Filed PMAT-492 (critical): trueno CUDA kernel cache. Real yoga throughput unknown — 2 hours = 98% JIT + 2% training. **WGPU path on Lambda RTX 4090 (119 tok/s, 3.7 min) remains the only practical measurement.** Next: trueno kernel caching investigation, lora_bwd buffer pool optimization. 60 upstream fixes. | PMAT-492 |
