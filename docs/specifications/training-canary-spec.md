...
| 4.9.0 | 2026-04-04 | **Per-op backward profiling wired + optimization sweep.** 6 backward ops instrumented with op_begin/op_end: OP_DOWN_BWD, OP_SWIGLU_BWD, OP_GATE_UP_BWD, OP_ATTN_BWD, OP_QKV_BWD, OP_NORM_BWD. Combined with 8 forward ops = 14/16 per-op coverage. `scripts/sweep.sh`: systematic 13-variant A/B measurement (7 isolated + 4 combined + max + baseline), each with profiling. Feeds into `scripts/parity-report.py` for comparison tables. 51 upstream fixes across 5 repos. The full optimization stack is now: attention backward (2.9x) + CUDA graph fwd+bwd (6.5x) + NF4 TC GEMM fwd+bwd (2.0x) = 37.7x expected. Measurement gap remains: need yoga/gx10 canary runs. | PMAT-483/488 |
