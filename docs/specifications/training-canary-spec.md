...
| 5.0.0 | 2026-04-04 | **Complete NF4 TC backward coverage + probar parity comparison.** All 7 projection backward GEMMs (Q, K, V, gate, up, down, O-proj) now dispatch to NF4 tensor core GEMM when `NF4_TC_BWD_GEMM=1`. Previously only FFN (3 projections) used TC — now attention (4 more) also uses TC. 28 layers × 7 projections = 196 TC backward GEMMs matching 196 forward TC GEMMs. probar parity comparison mode shipped: `compute_parity_comparison()` identifies per-op gaps between APR and baseline runtimes, `format_parity_markdown()` produces comparison tables. 52 upstream fixes across 5 repos. **The optimization stack is feature-complete** — all that remains is measurement. | PMAT-481/487 |
