...
**Parity gap (APR):** 421 tok/s wall (gx10 WGPU) vs 5,262 tok/s (unsloth, gx10) = **13x** on same hardware. **CRITICAL: GPU-compute parity ACHIEVED** — APR step-level throughput is 24,094 tok/s (4.6x faster than unsloth). The 13x gap is entirely inter-step CPU/sync overhead (99% GPU idle). See [optimization-roadmap.md PMAT-496](components/optimization-roadmap.md). Two blockers: (1) yoga WGPU blocked by missing `libvulkan.so.1` — install restores fast Q4K path; (2) gx10 binary still uses 20-min CPU dequant path — rebuild with `--gpu-backend wgpu` routing fix in progress. Upstream fix: aprender now respects `--gpu-backend wgpu` to force fast WGPU Q4K loading even when CUDA is available. See [optimization-roadmap.md](components/optimization-roadmap.md).
...
| 6.4.0 | 2026-04-05 | **GPU compute parity ACHIEVED — 24,094 tok/s step-level (4.6x faster than unsloth). 99% GPU idle.** Five-whys: (1) APR gx10 measures 421 tok/s wall but 24,094 tok/s at step level (85ms/step GPU vs 8,850ms/step wall). GPU is idle 99% of time between dispatches. (2) ROOT CAUSE: WGPU inter-step overhead — buffer allocation (784 `zeros()` calls/step), CPU-side data prep, WGPU command submission sync. (3) Unsloth gx10 same-hardware: 5,262 tok/s at batch=16. APR GPU compute is 4.6x faster. (4) The 13x wall-clock gap requires NO GPU optimization — fix is entirely CPU/sync: pre-allocate buffers, pipeline data loading, batch WGPU submissions. Expected: 8,850ms → ~100ms/step = 20,000+ tok/s. Filed PMAT-496. | PMAT-496 |
